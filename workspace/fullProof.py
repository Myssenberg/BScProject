...
#Generates other sks and pks, but voter only knows pks
...
R1_v_value = 1
R1_r_v_value = order.random()

c0, c1 = enc(g, order, pk_T, R1_v_value, R1_r_v_value) #changed, so that it is not the value but the secret 

enc_stmt1 = DLRep(c0, R1_r_v*g) & DLRep(c1, R1_v*g + R1_r_v*pk_T)

#Range proof for three candidates (vote is either 0, 1 or 2)
R1_v.value = R1_v_value
R1_r_v.value = R1_r_v_value
range_stmt = RangeStmt(c1, g, pk_T, 0, 3, R1_v, R1_r_v)

#Second proof of encryption
R1_lv_value = order.random() #this should maybe be something else? A list or something
R1_r_lv_value = order.random()

c2, c3 = enc(g, order, pk_vs, R1_lv_value, R1_r_lv_value)
...
R1_r_lvs_value = order.random()

reEnc = re_enc(g, order, pk_vs, (c4, g+c5), R1_r_lvs_value)
...
R2_r_v_value = order.random()

reEnc1 = re_enc(g, order, pk_T, (c0_v, c1_v), R2_r_v_value)
...
m_dec = dec(ct, R2_sk_value)

one = Secret(name="one", value=1)
neg_x = Secret(name="neg_x", value = -R2_sk_value)
neg_c0 = (-1)*c0
...
R2_r_lv_value = order.random()

reEnc2 = re_enc(g, order, pk_vs, ct_i, R2_r_lv_value)
...
R2_r_lvs_value = order.random()

reEnc3 = re_enc(g, order, pk_vs, ct_i, R2_r_lvs_value)
...
nizk = stmt.prove({R1_v: R1_v_value, R1_r_v: R1_r_v_value, R1_lv: R1_lv_value, R1_r_lv: R1_r_lv_value, R1_r_lvs: R1_r_lvs_value, R1_sk: R1_sk_value,
...
nizk = stmt.prove({R1_v: 0, R1_r_v: 0, R1_lv: 0, R1_r_lv: 0, R1_r_lvs: 0, R1_sk: 0,
                   R2_r_v: R2_r_v_value, R2_r_lv: R2_r_lv_value, R2_r_lvs: R2_r_lvs_value, R2_sk: R2_sk_value})
...
dec_stmt = DLRep(0*g, one*c1 + neg_x*c0)
